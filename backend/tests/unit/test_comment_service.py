...
class TestCreateComment:
    async def test_create_comment_nonexistent_task_raises(self, mock_db: AsyncMock):
    ...
    async def test_create_comment_with_invalid_parent_raises(self, mock_db: AsyncMock):
        mock_task = MagicMock()
        mock_task.id = 1
...
class TestGetComment:
    async def test_get_comment_not_found_raises(self, mock_db: AsyncMock):
        mock_db.scalar.return_value = None
...
class TestUpdateComment:
    async def test_update_comment_not_owner_raises(self, mock_db: AsyncMock):
...
class TestDeleteComment:
    async def test_delete_comment_not_owner_raises(self, mock_db: AsyncMock):
        mock_comment = MagicMock()
        mock_comment.id = 1
        mock_comment.user_id = 1
